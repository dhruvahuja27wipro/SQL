...
CREATE TEMPORARY TABLE temp_feed_with_pk (
    pk SERIAL PRIMARY KEY,  -- Postgres equivalent of AUTO_INCREMENT
    col_1 INT,
    col_2 VARCHAR(255),
    col_3 VARCHAR(255),
    col_4 VARCHAR(255),
...
-- Copy data from Feed1 into this temp table
...
-- Find duplicate rows by all 10 columns, keep only the first (lowest pk)
...
FROM (
    SELECT pk,
           ROW_NUMBER() OVER (
               PARTITION BY col_1, col_2, col_3, col_4, col_5, col_6, col_7, col_8, col_9, col_10
               ORDER BY pk
           ) as rn
...
-- Update duplicate rows with new random values
UPDATE temp_feed_with_pk
SET
    col_1 = pk,
    col_2 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_3 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_4 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_5 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_6 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_7 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_8 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_9 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8),
    col_10 = SUBSTRING(MD5(RANDOM()::TEXT), 1, 8)
WHERE pk IN (SELECT pk FROM temp_pks_to_update);

-- Refresh Feed1 with updated data
